...
util = {
    ...
    data: {
        ...
        embedReply: (message, type, msg) => {
            var e = new discord.RichEmbed();
            var col = settings_.success_color;
            var s = "Command Ran";
            switch (type) {
                case "error":
                    col = settings_.error_color;
                    s = "Command Error";
                    break;
                case "warn":
                    s = "Command Fail";
        ...
        embedServerList: async (msg) => {
            ...
            var e = embed();
            e.setColor(settings_.info_color);
        ...
        embedStaffList: async (msg) => {
            ...
            var c = Object.keys(msg.settings.roles).length;

            for (var rname in msg.settings.roles) {
                c--;
                ee[c] = msg.settings.roles[rname];
            }
            ...
            for (var pos in ee) {
                ...
                var role = g.roles.find(r => r.name == name);
            ...
            e.setColor(settings_.info_color);
            e.setDescription("All the people that can help you if you are having issues.");
            e.setTitle("Staff List");
            var d = Date();
            d = d.replace(" GMT+0000 (Coordinated Universal Time)", "");
            e.setFooter(`Last Update: ${d}`);
            return e;
        },
        embedRules: async (header, arr) => {
            var e = embed();
            e.setColor(settings_.info_color);
            e.setTitle("Server Rules");
        ...
        ticketLog: (msg, embed) => {
            ...
            var channel = g.channels.find(c => c.name == settings_.ticket_log && c.type == "text");

            if (channel) {
                channel.send(embed);
            } else {
                log("error", `Could not find ticket-log channel for guild ${g.name}`);
            }
        },
        client: () => {
            if (!cl) {
        ...
        setupLog: async (who, ticket) => {
            return save("tickets/" + who.id + ".txt", "--Beginning Of Chat Log For: " + ticket + "--", true)
        },
        getLog: async (who) => {
